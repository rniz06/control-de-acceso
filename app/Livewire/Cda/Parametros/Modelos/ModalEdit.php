<?php

namespace App\Livewire\Cda\Parametros\Modelos;

use App\Models\Cda\Marca;
use App\Models\Cda\Modelo;
use Illuminate\Support\Facades\Auth;
use Illuminate\Validation\Rule;
use Livewire\Attributes\Validate;
use Livewire\Component;

class ModalEdit extends Component
{
    /*
    |---------------------------------------------------------
    | Componente modal con formulario de edicion de modelo
    |---------------------------------------------------------
    */

    # PROPIEDAD PARA ALMACENAR EL REGISTRO A EDITAR
    public $registro;

    # PROPIEDADES DEL FORMULARIO
    #[Validate]
    public $modelo, $marca_id;

    # PROPIEDADES PARA LOS SELECT
    public $marcas = [];

    # FUNCION MOUNT DE LIVEWIRE
    public function mount($modelo_id)
    {
        $this->registro = Modelo::findOrFail($modelo_id);
        $this->modelo = $this->registro->modelo;;
        $this->marca_id = $this->registro->marca_id;

        $this->marcas  = Marca::get(['id', 'marca']);
    }

    # REGLAS DE VALIDACION
    protected function rules()
    {
        return [
            'modelo'   => ['required', 'string', 'max:30', Rule::unique(Modelo::class, 'modelo')->where('marca_id', $this->marca_id)->ignore($this->registro->id, 'id')],
            'marca_id' => ['required', Rule::exists(Marca::class, 'id')]
        ];
    }

    # FUNCION PARA GUARDAR EL NUEVO VEHICULO
    public function grabar()
    {
        $this->validate();
        $this->registro->update([
            'modelo'           => $this->modelo,
            'marca_id'         => $this->marca_id,
            'actualizado_por'  => Auth::id(),
        ]);

        session()->flash('success', 'Modelo Actualizado Correctamente!');
        $this->redirectRoute('cda.parametros.modelos.index');
    }

    public function render()
    {
        return view('livewire.cda.parametros.modelos.modal-edit');
    }
}
